...
class DownloadVoters:
    ...
    def get_districts(self,states,selected_option,get_districts_for_given_state,district_place):
        ...
        selected_district  = district_place.selectbox("Select a District", district_names,key="download_voter_1")
        # st.write(districts)
        # st.table(data)
    ...
    def get_constituencies(self,selected_option,
        ...
        selected_constituency = constituency_place.selectbox("Select a Constituency", constituencies_names,key="download_voter_11")
    ...
    def __init__(self,get_states: Callable[[str], bool],get_districts_for_given_state: Callable[[str], bool],get_constituencies_for_given_district: Callable[[str], bool],download_polling_booths: Callable[[str], bool]):
        ...
        if states is not None:
            ...
            Existing_State_Name = st.selectbox("Select a state", state_names,key="download_voter_2")
            ...
            if st.button('Download Voters',key="download_voter_3"):
                ...
                # st.write(data)
                file_name = "voters_"+Existing_State_Name+"_"+selected_district+"_"+selected_constituency+".csv"
                st.download_button(
                                    label="Download Voters as CSV",
                                    data=data,
                                    file_name=file_name,
                                    mime='text/csv',
                                )
