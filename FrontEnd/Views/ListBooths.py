...
class ListBooths:
    ...
    def __init__(self,get_states: Callable[[str], bool],get_districts_for_given_state: Callable[[str], bool],get_constituencies_for_given_district: Callable[[str], bool],list_polling_booths: Callable[[str], bool]):
        
        st.header("List Polling Booths")
        ...
        if states is not None:
            ...
            if st.button('View Polling Booths',key="update_con_3"):
                # st.write("State: ",Existing_State_Name)
                # st.write("DistrictName: ",selected_district)
                # st.write("Constituency: ",selected_constituency)
                data = list_polling_booths(Existing_State_Name,selected_district,selected_constituency)
                st.table(data)
