...
class ListVoters:
    ...
    def get_districts(self,states,selected_option,get_districts_for_given_state,district_place):
        ...
        selected_district  = district_place.selectbox("Select a District", district_names,key="edit_voter_1")
        # st.write(districts)
        # st.table(data)
    ...
    def get_constituencies(self,selected_option,
        ...
        selected_constituency = constituency_place.selectbox("Select a Constituency", constituencies_names,key="edit_voter_11")
    ...
    def __init__(self,get_states: Callable[[str], bool],get_districts_for_given_state: Callable[[str], bool],get_constituencies_for_given_district: Callable[[str], bool],list_voters: Callable[[str], bool]):
        
        st.header("List Voters")
        ...
        if states is not None:
            ...
            Existing_State_Name = st.selectbox("Select a state", state_names,key="update_voter_2")
            ...
            if st.button('View Voters',key="update_voter_3"):
                # st.write("State: ",Existing_State_Name)
                # st.write("DistrictName: ",selected_district)
                # st.write("Constituency: ",selected_constituency)
                data = list_voters(Existing_State_Name,selected_district,selected_constituency)
                st.table(data)
